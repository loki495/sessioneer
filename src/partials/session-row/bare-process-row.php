<li class="select-none rounded-xl border border-slate-800/60 bg-slate-900/30 px-4 py-3 flex items-start justify-between gap-3" data-bare-row data-pid="<?= $pid ?>" data-csrf-token="<?= $this->e($csrfToken) ?>">
  <div class="min-w-0 flex-1">
    <?php if (!empty($resolvedTitle)): ?>
      <div class="flex items-center gap-1.5">
        <span class="inline-block shrink-0 text-[10px] leading-none font-medium px-2 py-0.5 rounded-full border bg-emerald-900/30 text-emerald-400 border-emerald-700/40">Identified</span>
        <div class="text-sm truncate text-slate-300"><?= $this->e((string)$resolvedTitle) ?></div>
      </div>
    <?php elseif (!empty($title)): ?>
      <div class="text-sm truncate text-slate-300"><?= $this->e((string)$title) ?></div>
    <?php endif ?>
    ...
    <?php if ($hiddenWorkerCount > 0): ?>
      <div class="text-xs text-slate-500 mt-0.5">+<?= $hiddenWorkerCount ?> background helper process<?= $hiddenWorkerCount > 1 ? 'es' : '' ?> for this same session (not shown separately)</div>
    <?php endif ?>
  ...
  <div class="flex flex-col gap-2 shrink-0">
    ...
    <?php endif ?>
    <form method="post" action="/" onsubmit="return confirm('Kill pid <?= $pid ?><?= $tmuxSession !== null ? ' (tmux session ' . $this->e($tmuxSession) . ')' : '' ?>? This process was not started by this tool.<?= $hiddenWorkerCount > 0 ? ' Its ' . $hiddenWorkerCount . ' other background helper process(es) for this same session will NOT be killed by this.' : '' ?>');">
